...
treats as "the real implementation". If the first session's patch had leaked, which most pytest exit
paths do (usage errors, a conftest raising on import, `-h`, and others never reach
`pytest_collection`), the second session captured the first session's own wrapper as real, and every
cache lookup recursed into it forever.
...
`pytest_collection` reaches (it additionally covers `-h`), though still not all of them; what can
survive on the paths neither reaches is a wrapper that can no longer self-recurse, because it never
reads `functools.lru_cache` to find "the real implementation" in the first place.
